...
#
# Sample Usage :
#  third_party::sysctl::sysctl { 'net.ipv6.bindv6only': value => '1' }
#
define third_party::sysctl::sysctl ($value = undef, $prefix = undef, $comment = undef, $ensure = undef,) {
  include third_party::sysctl::base
...
# The permanent change
  file { "/etc/sysctl.d/${sysctl_d_file}":
    ...
    group   => 'root',
    mode    => '0644',
    content => template("third_party/sysctl/sysctl.d-file.erb"),
    notify  => [Exec["sysctl-${title}"], Exec["update-sysctl.conf-${title}"],],
  }
